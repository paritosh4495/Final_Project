...
# # Img1
        ...
        title_lbl = Label(bg_img, text="FACE RECOGNITION ",font=("helvetica", 25,"bold"),bg="Beige",fg="black")
        title_lbl.place(x=0,y=0,width=1530,height=45)

# img 2
        img_bottom=Image.open(r"Images\face_recogR.jpg")
        img_bottom=img_bottom.resize((950,325),Image.Resampling.LANCZOS) 
        self.photoimg_bottom=ImageTk.PhotoImage(img_bottom)

        f_lb1=Label(self.root,image=self.photoimg_bottom)
        f_lb1.place(x=425,y=185,width=950,height=550)

        b1_1=Button(f_lb1,text="START FACE RECOGNITION",command=self.face_recog,cursor="hand2",font=("helvetica", 18,"bold"),bg="black",fg="white")
        b1_1.place(x=50,y=445,width=400,height=40)
    ...
    def mark_attendance(self,i,r,n,d):
        with open("bvcoel.csv","r+",newline='\n') as f:
            myDataList=f.readlines()
            name_list=[]
            for line in myDataList:
                entry=line.split((","))
                name_list.append(entry[0])
            #this is so that it doesnt captures multiple times one person
        ...
        def draw_boundray(img,classifier,scaleFactor,minNeighbors,color,text,clf):
            ...
            for (x,y,w,h) in features:
                ...
                if confidence>77:
                    ...
                    cv2.putText(img,"Unknown Face: {d}",(x,y-5),cv2.FONT_HERSHEY_COMPLEX,0.8,(255,255,255),3)
